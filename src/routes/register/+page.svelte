<script lang="ts">
  ...
  // Validation helpers
  // Celo: Self manages the agent key internally — agent address not required
  // Base: agent address is optional (linked via setAgentWallet post-mint)
  $: step2Valid = !!$walletAddress && (network === 'celo' || /^0x[a-fA-F0-9]{40}$/.test(agentAddressInput)) && (network !== 'base' || siweVerified);
        ...
        <div class="step-content">
          ...
          <div class="network-cards">
            <button class="network-card" on:click={() => selectNetwork('celo')}>
              ...
              <p>Register with verified proof-of-human on Celo via Self Protocol (passport NFC + ZK proof).</p>
      ...
      {:else if step === 1}
        <div class="step-content">
          ...
          <div class="card section-card info-card" class:disabled={!$walletAddress}>
            <h3 class="section-label">3. Agent address <span class="optional">(optional)</span></h3>
            <p class="muted-text">Your agent's operational ECDSA keypair address. If you are an agent, generate a keypair and paste your address here. You can also skip this and link it later.</p>
            <a href="/skill.md" class="muted-link">&rarr; How agents self-register (skill.md)</a>
            <div class="form-group" style="margin-top: 1rem;">
              <label class="form-label" for="agentAddress">Agent address <span class="optional">(optional)</span></label>
              <input
                id="agentAddress"
                type="text"
                class="form-input"
      ...
      {:else if step === 3}
        <div class="step-content">
          ...
          <div class="card summary-card">
            ...
            {#if network === 'base' && agentAddressInput}
            <div class="summary-row"><span>Agent address</span> <code class="mono">{truncate(agentAddressInput)}</code></div>
            {/if}
            {#if network === 'celo'}
            <div class="summary-row"><span>Agent key</span> <span class="muted-text" style="font-size:0.8rem">Generated by Self Protocol during scan</span></div>
            {/if}
